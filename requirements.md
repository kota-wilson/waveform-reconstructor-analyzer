...
| WRA-RQ-036 | The config model shall support measurement-backed DSL criteria. | v0.5.0 / issue #55 | High | TOML supports `[criteria.measurement]` and `[criteria.requirement]` sections for existing measurement types without breaking existing criteria configs. | Software Architect / Core Software Engineer | Implemented in branch |
