# ferrisoxide-embedded no_std Design

Date: 2026-05-31

## Purpose

`ferrisoxide-embedded` defines the runtime adapter boundary that sits between platform-specific embedded code and `ferrisoxide-signal`.
...
Platform-specific source, sink, and runtime errors remain associated types. `AdapterError` keeps those errors distinct from `ferrisoxide_signal::SignalError` so hardware/runtime failures do not look like signal-analysis failures.
