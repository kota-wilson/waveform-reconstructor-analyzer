...
fn evaluate_criterion(waveform: &Waveform, criterion: &Criterion) -> Result<AnalysisResult> {
    ...
    match &criterion.check {
        ...
        CriterionCheck::StateTransitions {
            ...
            evaluate_state_transitions(waveform, channel, criterion, *threshold_v, *expected_count)
        }
...
fn evaluate_state_transitions(
    ...
    let transitions = transition_indices(channel, threshold_v);
...
fn evaluate_pulse_width(
    ...
    if min_width_s.is_none() && max_width_s.is_none() {
    ...
    if state_runs.is_empty() {
        return Ok(result(
            ...
            Evidence {
                measured_value: 0.0,
                required_value: min_width_s.or(max_width_s).unwrap_or_default(),
                unit: "s",
                sample_index: 0,
    ...
    if let Some(min_width_s) = min_width_s {
        ...
        if shortest.duration_s + FLOAT_TOLERANCE < min_width_s {
            return Ok(result(
                criterion,
                Outcome::Fail,
                Evidence {
    ...
    if let Some(max_width_s) = max_width_s {
        ...
        if longest.duration_s - FLOAT_TOLERANCE > max_width_s {
            return Ok(result(
                criterion,
                Outcome::Fail,
                Evidence {
...
fn evaluate_transient_duration(
    ...
    let outcome = if measured <= max_duration_s + FLOAT_TOLERANCE {
        Outcome::Pass
    } else {
        Outcome::Fail
    };
...
fn evaluate_stable_state_duration(
    ...
    let (measured, sample_index, timestamp) = longest
        .map(|run| (run.duration_s, run.start_index, run.start_time))
        .unwrap_or((0.0, 0, waveform.time[0]));
    let outcome = if measured + FLOAT_TOLERANCE >= min_duration_s {
        Outcome::Pass
    } else {
        Outcome::Fail
    };
...
fn evaluate_rise_fall_time(
    ...
    let measurement = match direction {
        EdgeDirection::Rise => measure_rise(waveform, channel, low_threshold_v, high_threshold_v),
        EdgeDirection::Fall => measure_fall(waveform, channel, low_threshold_v, high_threshold_v),
    };
    ...
    let outcome = if observed && measured <= max_duration_s + FLOAT_TOLERANCE {
        Outcome::Pass
    } else {
        Outcome::Fail
    };
...
fn transition_indices(channel: &Channel, threshold_v: f64) -> Vec<usize> {
    channel
        .samples
        .windows(2)
        .enumerate()
        .filter_map(|(index, pair)| {
            let previous = sample_state(pair[0], threshold_v);
            let next = sample_state(pair[1], threshold_v);
...
fn state_runs(waveform: &Waveform, channel: &Channel, threshold_v: f64) -> Vec<StateRun> {
    let mut runs = Vec::new();
    let mut start_index = 0;
    let mut current_state = sample_state(channel.samples[0], threshold_v);

    for index in 1..channel.samples.len() {
        let state = sample_state(channel.samples[index], threshold_v);
...
fn sample_state(value: f64, threshold_v: f64) -> SignalState {
...
fn measure_rise(
    ...
    let start_index = channel
        .samples
        .iter()
        .position(|value| *value >= low_threshold_v)?;
    let end_index = channel
        .samples
        .iter()
        .enumerate()
        .skip(start_index)
        .find_map(|(index, value)| (*value >= high_threshold_v).then_some(index))?;
...
fn measure_fall(
    ...
    let high_index = channel
        .samples
        .iter()
        .position(|value| *value >= high_threshold_v)?;
    let start_index = channel
        .samples
        .iter()
        .enumerate()
    ...
    let end_index = channel
        ...
        .enumerate()
        .skip(start_index)
        .find_map(|(index, value)| (*value <= low_threshold_v).then_some(index))?;
    Some(TransitionMeasurement {
        end_index,
        end_time: waveform.time[end_index],
        duration_s: waveform.time[end_index] - waveform.time[start_index],
