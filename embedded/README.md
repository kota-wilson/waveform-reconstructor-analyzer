...
1. `crates/ferrisoxide-signal`: no_std signal primitives.
2. `embedded/arm64/qemu`: ARM64 QEMU proof.
3. `crates/ferrisoxide-embedded`: RTOS/ARM64 adapter layer.
4. `embedded/arm64/zephyr`: optional feasibility prototype.

Current status:

- `crates/ferrisoxide-signal`: implemented by M3-RTOS-001.
- `crates/ferrisoxide-embedded`: implemented by M3-RTOS-003 as a no_std adapter boundary.
