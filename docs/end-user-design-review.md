...
| EDR-002 | Electrical Signal Integrity Engineer | High | `Waveform` now records source name, units, sample count, channel metadata, sample interval, nominal sample rate, lineage, and transform history; tolerances, calibration, scale factor, and uncertainty remain future work. | Add a v0.3.0 tolerance and validation model before making stronger signal-accuracy claims. | Software Architect | M4 validation roadmap. |
...
| P1 | Add waveform metadata model for source, units, sample counts, sample interval, lineage, and transform history. | Issue #4. |
| P1 | Add tolerance and acquisition-context metadata after known-answer validation starts. | M4-003 and M4-006. |
