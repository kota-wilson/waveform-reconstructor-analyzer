...
| `ferrisoxide-deployment` | `crates/ferrisoxide-deployment` | Reviewable controller/RTOS deployment package manifest schema and validator. | Deployment package metadata, target profile metadata, mode profiles, required artifact roles, artifact validation, production/test/signal-validation mode policy validation, separated production/test config validation, and checksum index wording helper. |
...
| `ferrisoxide-deployment` | `crates/ferrisoxide-deployment/src/lib.rs` | Deployment package manifest schema, mode-profile schema, and validation helpers; no export command, runtime loader, signing, authentication, HAL, SDK, target hardware, or certification behavior. |
