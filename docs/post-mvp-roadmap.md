...
Status: Planning backlog after MVP exit, the M21-M24 runtime-path follow-up, the complete M25-M36 comprehensive filter/signal-conditioning suite merged through PR #175, and the locally implemented M37-M42 desktop user workflow path. Release publication, dependency additions, runtime-loader implementation, live DAQ, HAL/RTOS, target hardware, GUI, and certification scope remain separately gated.
...
The completed selected planning theme was the comprehensive sampled-waveform filter and signal-conditioning suite. The current local implementation theme is the desktop user workflow path: run the program, choose CSV/simulated/future-gated realtime source, identify and label channels, apply transforms and filters, add criteria, run evaluation, and review results.
...
| Desktop user workflow path | M37-M42 roadmap for source intake, channel labeling, transform/criteria authoring, evaluation bundles, result review, and validation corpus coverage. | Implemented locally; see `docs/desktop-user-workflow.md` and `docs/desktop-user-workflow-roadmap.md` |
...
| Desktop source inspection and scaffolding | CSV/source inspection, channel/unit detection, starter TOML config generation, simulation source inspection through channel maps, and unsupported realtime-mode guardrails. | Implemented locally for CSV and fixture simulation; live/realtime/hardware channel discovery remains gated |
| Evaluation bundle workflow | One predictable results directory with source summary, config copy, text/JSON report or simulation evidence, optional SVG, summary, and failure-triage notes. | Implemented locally for CSV and fixture simulation; broader batch-corpus automation remains candidate work |
...
- No GUI, live/realtime DAQ, vendor SDK, hardware acquisition, runtime loader, release publication, or certification workflow has been implemented by the M37-M42 local workflow work.
...
Validate and merge the M38-M42 desktop workflow implementation after local checks. M25 created the transform registry and completeness contract, M26 added data-cleaning/timing-conditioning transforms, M27 added pointwise normalization/nonlinear transforms, M28 added smoothing/baseline conditioning transforms, M29 added standard frequency filters, M30 added resampling/timing-alignment transforms, M31 added envelope/energy/calculus filters and feature records, M32 added statistics/correlation filters and feature records, M33 added spectrum/window/time-frequency feature records, M34 added deterministic fault injection and ADC/DAC simulation, M35 added multi-channel, sensor, vibration, and control conditioning, M36 closed catalog/docs/corpus/compatibility/readiness evidence before the PR #175 merge, and M37-M42 now implement the CLI-first workflow tying source inspection, config scaffolding, templates, evaluation bundles, and validation fixtures together.
...
Status: Backlog separated; M21-M24 runtime-path follow-up and M25-M36 comprehensive suite work are complete and merged through PR #175; M37-M42 desktop workflow path is implemented and validated locally, with merge still handled through the standard process.
...
Next recommended step: Merge the workflow documentation and CLI implementation through the standard process.
