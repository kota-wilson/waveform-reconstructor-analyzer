...
| WRA-TASK-080 | Record architecture diagram hierarchy rule | Documentation Engineer / Software Architect | User documentation-rule recommendation | `docs/architecture/ferrisoxide-overview.md` defines the high-level system flowchart; `AGENTS.md` and `docs/architecture.md` define major crate criteria, focused crate-local `architecture.md` requirement, Mermaid component/data-flow diagram requirement, no-duplicate-system-diagram rule, and existing-crate backfill boundary | Documentation Gate | Complete for rule; backfill pending |
